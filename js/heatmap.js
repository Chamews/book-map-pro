...
const MAX_BUBBLES = 200000; 
const MAX_CVD = 200000;
...
let priceZoom = parseFloat(localStorage.getItem('heatmapPriceZoom')) || 1.0;
...
let lastMouseY = 0;
let lastMouseX = 0;
let sidebarW = 180; // Dynamic sidebar width
let timeOffset = 0; // ms offset from "now"
let isLive = true;

// 🎨 DRAWING TOOLS STATE
let drawings = [];
let activeDrawingTool = null; // 'trendline', 'box', 'hline', 'delete'
let isDrawing = false;
let currentDrawing = null;
let drawingStartPoint = null; // { time, price }

// 📊 CVD SCALING STATE (For Pane-Aware Drawings)
let currentCVDMin = 0;
let currentCVDMax = 1;
...
function toggleMute() {
    ...
    const btns = [document.getElementById('muteBtn'), document.getElementById('mVolumeIcon')];
    btns.forEach(btn => {
        if (btn) {
            btn.innerHTML = isMuted ? '🔇' : '🔊';
            if (btn.id === 'muteBtn') btn.style.opacity = isMuted ? '0.4' : '0.8';
        }
    });
...
function updateBubbleScale(val) {
    ...
    // Sync UI
    const sliders = ['bubbleSizeSlider', 'proSize', 'mSizeSlider'];
    sliders.forEach(id => {
        const s = document.getElementById(id);
        if (s) s.value = val;
    });
    const valDisplay = document.getElementById('mSizeVal');
    if (valDisplay) valDisplay.innerText = bubbleScale.toFixed(1);
}
...
function updateHeatSensitivity(val) {
    ...
    // Sync UI
    const sliders = ['heatFilterSlider', 'proHeat', 'mHeatSlider'];
    sliders.forEach(id => {
        const s = document.getElementById(id);
        if (s) s.value = val;
    });
    const valDisplay = document.getElementById('mHeatVal');
    if (valDisplay) valDisplay.innerText = heatSensitivity.toFixed(2);
}
...
function updateTimeZoom(val) {
    ...
    // Sync sliders
    const sliders = ['proZoomX', 'timeZoomSlider', 'mZoomXSlider'];
    sliders.forEach(id => {
        const s = document.getElementById(id);
        if (s) s.value = val;
    });
    const valDisplay = document.getElementById('mZoomXVal');
    if (valDisplay) valDisplay.innerText = val + 's';
}
...
function updatePriceZoom(val) {
    ...
    localStorage.setItem('heatmapPriceZoom', val);
    
    // Sync UI
    const sliders = ['proZoomY', 'priceZoomSlider', 'mZoomYSlider'];
    sliders.forEach(id => {
        const s = document.getElementById(id);
        if (s) s.value = val;
    });
    const valDisplay = document.getElementById('mZoomYVal');
    if (valDisplay) valDisplay.innerText = priceZoom.toFixed(1);
}
...
function setDrawingTool(tool) {
    ...
    if (activeDrawingTool) {
        const activeBtn = document.getElementById(`btn-${activeDrawingTool}`);
...
function clearAllDrawings() {
    if (confirm('Clear all drawings on chart?')) {
        drawings = [];
    }
}
...
async function openHeatmap(pair) {
    ...
    // Sync UI Sliders
    document.querySelectorAll('#bubbleSizeSlider, #proSize, #mSizeSlider').forEach(s => s.value = bubbleScale);
    document.querySelectorAll('#heatFilterSlider, #proHeat, #mHeatSlider').forEach(s => s.value = heatSensitivity);
    document.querySelectorAll('#timeZoomSlider, #proZoomX, #mZoomXSlider').forEach(s => s.value = TIME_WINDOW_MS / 1000);
    document.querySelectorAll('#priceZoomSlider, #proZoomY, #mZoomYSlider').forEach(s => s.value = priceZoom);
    document.querySelectorAll('#domAggSlider').forEach(s => s.value = domAggregation);
    
    // Update mobile text displays
    if (document.getElementById('mSizeVal')) document.getElementById('mSizeVal').innerText = bubbleScale.toFixed(1);
    if (document.getElementById('mHeatVal')) document.getElementById('mHeatVal').innerText = heatSensitivity.toFixed(2);
    if (document.getElementById('mZoomXVal')) document.getElementById('mZoomXVal').innerText = (TIME_WINDOW_MS / 1000) + 's';
    if (document.getElementById('mZoomYVal')) document.getElementById('mZoomYVal').innerText = priceZoom.toFixed(1);
    if (document.getElementById('mVolumeIcon')) document.getElementById('mVolumeIcon').innerText = isMuted ? '🔇' : '🔊';
    
    // Update Heatmap checkbox (Default OFF)
    const showHeat = document.getElementById('showHeat');
    if (showHeat) showHeat.checked = false;
    
    // Update mobile candle btn state
    const showCandles = document.getElementById('showCandles');
    if (showCandles) showCandles.checked = false; // Default OFF
    const mCandleBtn = document.getElementById('mCandleBtn');
    if (showCandles && mCandleBtn) mCandleBtn.classList.remove('active');

    // Sync CVD toggle state
    const showCVD = document.getElementById('showCVD');
    const mCvdToggle = document.getElementById('mCvdToggle');
    if (showCVD && mCvdToggle) mCvdToggle.checked = showCVD.checked;
    ...
    (function forceSizeCanvases() {
        ...
        if (heatCanvas) {
            const parentW = (heatCanvas.parentElement && heatCanvas.parentElement.offsetWidth) || (dashW - sidebarW - 5) || (window.innerWidth * 0.88);
        ...
        if (domCanvas) {
            const domParentW = (domCanvas.parentElement && domCanvas.parentElement.offsetWidth) || sidebarW;
        ...
        if (timeCanvas) {
            const timeParentW = (timeCanvas.parentElement && timeCanvas.parentElement.offsetWidth) || (dashW - sidebarW - 5);
    ...
    loadTradeHistory(pair, 30000).catch(e => console.error('[Heatmap] Trade history err:', e));
...
async function refreshDeepDepth(pair) {
    ...
    try {
        ...
        // Try Binance directly first if on static host or as a reliable fallback
        try {
            const bRes = await fetch(`https://fapi.binance.com/fapi/v1/depth?symbol=${symbol}&limit=1000`);
            if (bRes.ok) data = await bRes.json();
        } catch (e) { 
            console.warn("[Heatmap] Direct Binance fetch failed"); 
        }

        if (!data && isBackendAvailable) {
            const res = await fetch(`http://localhost:3000/api/liquidity/proxy-depth/${symbol}`);
        ...
        if (data && data.bids) {
            ...
            // 🎯 FOR HEATMAP: Keep TOP 400 levels for high-fidelity background depth
            const topBids = [...allBids].sort((a, b) => b[1] - a[1]).slice(0, 400);
            const topAsks = [...allAsks].sort((a, b) => b[1] - a[1]).slice(0, 400);
            ...
            console.log(`[Heatmap] ✅ Pro-Depth Synced: Raw ${allBids.length + allAsks.length}L | High-Density ${topBids.length + topAsks.length}L for ${symbol}`);
        }
...
function setupInteractions() {
    ...
    window.onmousedown = (e) => {
        ...
        // 🎨 DRAWING LOGIC START
        if (activeDrawingTool && e.target === canvas) {
            const now = Date.now();
            const endTimeVisible = isLive ? now : (now + timeOffset);
            const startTimeVisible = endTimeVisible - TIME_WINDOW_MS;
            
            const pTime = startTimeVisible + (mX / (canvas.width / dpr)) * TIME_WINDOW_MS;
            
            // 🎯 PANE DETECTION (Main vs CVD)
            const h = canvas.height / dpr;
            const cvdAreaH = Math.min(120, h * 0.2);
            const cvdBTM = h - 30;
            const cvdTop = cvdBTM - cvdAreaH;
            
            let pPrice = minPrice + (1 - mY / h) * (maxPrice - minPrice);
            let drawingPane = 'main';

            if (mY >= cvdTop && mY <= cvdBTM) {
                drawingPane = 'cvd';
                // Normalize price relative to CVD scale
                pPrice = currentCVDMin + (1 - (mY - cvdTop) / cvdAreaH) * (currentCVDMax - currentCVDMin);
            }

            if (activeDrawingTool === 'delete') {
                // Find and delete nearest drawing
                let nearestIdx = -1;
                let minDist = 30; // Max 30 pixels distance
                
                drawings.forEach((d, idx) => {
                    const x1 = ((d.p1.time - startTimeVisible) / TIME_WINDOW_MS) * (canvas.width / dpr);
                    const y1 = (canvas.height / dpr) - ((d.p1.price - minPrice) / (maxPrice - minPrice) * (canvas.height / dpr));
                    
                    if (d.type === 'hline') {
                        const dist = Math.abs(mY - y1);
                        if (dist < minDist) { minDist = dist; nearestIdx = idx; }
                    } else {
                        const x2 = ((d.p2.time - startTimeVisible) / TIME_WINDOW_MS) * (canvas.width / dpr);
                        const y2 = (canvas.height / dpr) - ((d.p2.price - minPrice) / (maxPrice - minPrice) * (canvas.height / dpr));
                ...
                if (nearestIdx !== -1) drawings.splice(nearestIdx, 1);
                return;
            }
            ...
            if (activeDrawingTool === 'hline') {
                drawings.push({
                    id: Date.now(),
                    type: 'hline',
                    p1: { time: pTime, price: pPrice },
                    p2: { time: pTime + 1000, price: pPrice },
                    color: '#3b82f6'
                });
                return;
            }
            ...
            isDrawing = true;
            dragM = 4; // Special mode for drawing
            drawingStartPoint = { time: pTime, price: pPrice };
            currentDrawing = {
                type: activeDrawingTool,
                pane: drawingPane,
                p1: drawingStartPoint,
                p2: drawingStartPoint,
                color: '#3b82f6'
            };
            return;
        } else if (e.target === canvas) {
            // If click is on the right side (price scale / sidebar), zoom Y
            if (mX > canvas.offsetWidth - sidebarW - 10) {
                dragM = 2; // Zoom Y
    ...
    canvas.ontouchstart = (e) => {
        ...
        if (e.touches.length === 1) {
            ...
            // Increase hit area for vertical zoom on mobile (entire sidebar area)
            if (mX > canvas.offsetWidth - sidebarW - 10) {
                dragM = 2; // Zoom Y
            } else {
                dragM = 1; // Pan
    ...
    window.onmousemove = (e) => {
        ...
        if (isDrawing && currentDrawing) {
            const endTimeVisible = isLive ? Date.now() : (Date.now() + timeOffset);
            const startTimeVisible = endTimeVisible - TIME_WINDOW_MS;
            const dpr = window.devicePixelRatio || 1;
            ...
            let p2Price = minPrice + (1 - mY / h) * (maxPrice - minPrice);
            if (currentDrawing.pane === 'cvd') {
                p2Price = currentCVDMin + (1 - (mY - cvdTop) / cvdAreaH) * (currentCVDMax - currentCVDMin);
            }
            
            currentDrawing.p2 = {
                time: startTimeVisible + (mX / w) * TIME_WINDOW_MS,
                price: p2Price
            };
            return;
        }
    ...
    window.onmouseup = () => {
        if (isDrawing && currentDrawing) {
            drawings.push({ ...currentDrawing, id: Date.now() });
            isDrawing = false;
            currentDrawing = null;
        }
        dragM = 0;
...
async function loadInitialHistory(pair) {
    try {
        ...
        const isBackendAvailable = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
        if (!isBackendAvailable) return; // Skip history if no backend

        const res = await fetch(`http://localhost:3000/api/liquidity/history/${pairParam}`);
        const data = await res.json();
        if (data.success) {
...
async function loadTradeHistory(pair, limit = 1000, targetEndTime = null) {
    if (isLoadingHistory && targetEndTime) return; // Prevent concurrent history loads
    if (targetEndTime) isLoadingHistory = true;

    try {
        const symbol = pair.replace('/', '').toUpperCase();
        let allTrades = [];
        let endTime = targetEndTime || Date.now();

        // Fetch in chunks of 1000 (Binance limit)
        const fetchLimit = 1000;
        const totalToFetch = Math.min(limit, 10000); // Increased max fetch per call
        ...
        let aggregated = []; 
        // Only reset CVD if we are loading initial history (no targetEndTime)
        if (!targetEndTime) {
            currentCVD = 0; 
            cvdData = [];
        }
        ...
        allTrades.forEach(t => {
            ...
            if (last && last.time === timeBin) {
                ...
                aggregated.push({ 
                    ...
                    buyQty: t.m ? 0 : parseFloat(t.q), 
                    sellQty: t.m ? parseFloat(t.q) : 0 
                });
            }
        });

        // Merge with existing tradeBubbles
        ...
        // Re-calculate CVD for the entire buffer to keep it consistent
        if (tradeBubbles.length > 0) {
            let tempCVD = 0;
            cvdData = [];
            tradeBubbles.forEach(b => {
                tempCVD += (b.buyQty - b.sellQty);
                cvdData.push({ time: b.time, value: tempCVD });
            });
            currentCVD = tempCVD;
        }
...
function startWebSockets(pair) {
    ...
    const wsUrl = `wss://fstream.binance.com/public/ws/${symbol}@depth50@100ms`;
    ...
    const tradeWsUrl = `wss://fstream.binance.com/market/ws/${symbol}@aggTrade`;
...
function checkAndLoadMoreBubbles() {
    if (!heatmapActive || !heatmapPair || isLoadingHistory || Date.now() - lastHistoryFetchTime < 1000) return;
    ...
    if (tradeBubbles.length > 0) {
        const oldestTradeTime = tradeBubbles[0].time;
        // If visible start is earlier than oldest trade, or if we have very few trades before the start
        if (startTimeVisible < oldestTradeTime) {
            // Load a larger chunk (10,000) when lazy-loading to avoid frequent requests
            loadTradeHistory(heatmapPair, 10000, oldestTradeTime - 1);
        }
    }
}

function renderDrawings(ctx, getX, getY, w, h) {
    if (drawings.length === 0 && !isDrawing) return;

    ctx.save();
    
    // Draw saved drawings
    drawings.forEach(d => {
        drawSingleDrawing(ctx, d, getX, getY, w, h);
    });

    // Draw current drawing if active
    if (isDrawing && currentDrawing) {
        ctx.globalAlpha = 0.6;
        drawSingleDrawing(ctx, currentDrawing, getX, getY, w, h);
    }

    ctx.restore();
}

function drawSingleDrawing(ctx, d, getX, getY, w, h) {
    let x1 = getX(d.p1.time);
    let y1 = getY(d.p1.price);
    let x2 = getX(d.p2.time);
    let y2 = getY(d.p2.price);
    ...
    if (d.pane === 'cvd') {
        ...
        const cvdBTM = h - 30;
        const cvdTop = cvdBTM - cvdAreaH;
        const getCVdy = (val) => cvdTop + (1 - (val - currentCVDMin) / (currentCVDMax - currentCVDMin)) * cvdAreaH;
        y1 = getCVdy(d.p1.price);
        y2 = getCVdy(d.p2.price);
    }
    ...
    if (d.type === 'trendline') {
        ...
        ctx.fillStyle = (d.color || '#3b82f6') + '22'; // Add alpha
...
function drawMainChart() {
    ...
    if (isLoadingHistory) {
        ...
        ctx.textAlign = 'left';
        ctx.fillText('⏳ LOADING HISTORY...', 10, 25);
    }
    ...
    // 🔑 Vertical Auto-Scaling (Only if we are in LIVE mode)
    if (isLive) {
        if (currentPrice > 0) {
    ...
    // 🧱 HISTORICAL HEATMAP ENGINE (Trailing Liquidity)
    const drawHistoricalHeatmap = () => {
        if (!visibleSnaps || visibleSnaps.length < 1) return;

        const sidebarW = 180;
        const chartW = w - sidebarW;
        const filterThreshold = 0.2 + (heatSensitivity * 0.6); 
        const maxQty = globalMaxQty || 1;

        // --- 🧱 INSTITUTIONAL TRAILING EDGE ENGINE ---
        const renderLevels = (levels, drawX, sliceW, alpha = 0.7) => {
            // Aggregate levels into ticks to prevent "boxy" vertical overlaps and improve performance
            const binnedSlice = new Map();
            
            levels.forEach(l => {
                const price = l[0];
                const qty = l[1];
                if (price < minPrice - tickSize || price > maxPrice + tickSize || qty <= 0) return;

                const binPrice = Math.round(price / tickSize) * tickSize;
                binnedSlice.set(binPrice, (binnedSlice.get(binPrice) || 0) + qty);
            });

            binnedSlice.forEach((qty, binPrice) => {
                const intensity = Math.min(1, Math.pow(qty / maxQty, 1.8));
                if (intensity < filterThreshold) return;

                const y = getY(binPrice);
                const color = getVividColor(intensity, alpha);
                ctx.fillStyle = color;

                let blockH = (tickSize / (maxPrice - minPrice)) * h;
                blockH = Math.max(1.5, Math.min(h * 0.08, blockH));
                
                // sliceW + 1.2 to absolutely eliminate sub-pixel gaps on high-refresh displays
                ctx.fillRect(drawX - 0.5, y - blockH/2, sliceW + 1.2, blockH);
            });
        };

        // 1. Draw Historical Slices
        for (let i = 0; i < visibleSnaps.length - 1; i++) {
            const s1 = visibleSnaps[i];
            const s2 = visibleSnaps[i + 1];
            
            const x1 = getX(s1.time);
            const x2 = getX(s2.time);
            
            if (x2 < -50 || x1 > chartW) continue;

            const sliceW = Math.max(1, x2 - x1);
            const drawX = x1;
            
            const clippedX = Math.max(0, drawX);
            const clippedW = Math.min(sliceW, chartW - clippedX);

            if (clippedW > 0) {
                renderLevels(s1.bids, clippedX, clippedW);
                renderLevels(s1.asks, clippedX, clippedW);
            }
        }

        // 2. 🚀 CRITICAL: LEADING EDGE FILL (Synchronize Liquidity to "Now" Line)
        // This ensures the heatmap background always reaches the trade bubbles
        if (visibleSnaps.length > 0) {
            const lastSnap = visibleSnaps[visibleSnaps.length - 1];
            const lastX = getX(lastSnap.time);
            const nowX = getX(endTimeVisible);
            
            if (nowX > lastX && lastX < chartW) {
                const fillX = Math.max(0, lastX);
                const fillW = Math.min(nowX - fillX, chartW - fillX);
                
                if (fillW > 0) {
                    // Use slightly higher alpha or solid for the lead edge to feel "Live"
                    renderLevels(lastSnap.bids, fillX, fillW, 0.8);
                    renderLevels(lastSnap.asks, fillX, fillW, 0.8);
                }
            }
        }
    };
    ...
    // 🔥 DRAW HISTORICAL HEATMAP FIRST (Background Layer)
    if (document.getElementById('showHeat')?.checked) {
        drawHistoricalHeatmap();
    }
    ...
    ctx.fillRect(chartW, 0, sidebarW, h);
    ...
    if (heatToggle && heatToggle.checked) {
        ...
        domLevels.forEach(l => {
            ...
            ctx.fillRect(0, y - 5, chartW, 10);

            // B. LEADING EDGE INDICATOR (Small protrusion into the chart)
            ...
            ctx.fillRect(chartW - 30, y - wallH / 2, 30, wallH);

            // C. ELITE GLOW CORE (Solid Center)
            if (intensity > 0.8) {
                ctx.fillStyle = color;
                ctx.fillRect(chartW - 20, y - 1, 20, 2);
            }

            // D. SIDEBAR VOLUME BAR (Right side of canvas)
            const barMaxW = sidebarW - 70;
    ...
    if (document.getElementById('showPOC')?.checked) {
        ...
        if (pocPrice > 0) {
            ...
            ctx.fillStyle = '#8b5cf6'; ctx.font = 'bold 11px Inter'; ctx.fillText(`POC: $${pocPrice}`, chartW - 85, y - 5);
        }
    }

    // ⚡ IMBALANCE MARKERS (Optimized visible slice)
    ...
    if (document.getElementById('showBubbles')?.checked) {
        ...
        const visibleTrades = [];
        const rightEdgeLimit = chartW + 50;
        let maxVol = 1;
        let totalVolSum = 0;
        ...
        visibleTrades.forEach(t => {
            const x = getX(t.time);
            if (x > chartW || x < -50) return; // Boundary clip for sidebar alignment
            const y = getY(t.price);
            const totalQty = t.buyQty + t.sellQty;
            const delta = t.buyQty - t.sellQty;

            // 💡 SCALING: The largest bubble on screen gets baseFactor = 1.0
            const baseFactor = Math.sqrt(totalQty / maxVol);
            let radius = baseFactor * (35 * bubbleScale);
            radius = Math.min(90, Math.max(1.2, radius)); // Lowered min to show almost all trades

            // PERFORMANCE & NOISE: For very tiny dots, draw simple circles to save GPU
            if (radius < 2.5) {
            ...
            // --- 🧪 PRO 3D SPHERE ENGINE ---
            
            // 1. Draw solid base (Two Halves)
            if (buyRatio > 0) {
                ctx.fillStyle = '#01a65a'; // Slightly darker green base
                ctx.beginPath(); ctx.moveTo(x, y); ctx.arc(x, y, radius, startAngle, buyEndAngle); ctx.closePath(); ctx.fill();
            }
            if (buyRatio < 1) {
                ctx.fillStyle = '#c0392b'; // Slightly darker red base
                ctx.beginPath(); ctx.moveTo(x, y); ctx.arc(x, y, radius, buyEndAngle, startAngle + Math.PI * 2); ctx.closePath(); ctx.fill();
            }

            // 2. High-Performance Spherical Highlight (No 'source-atop' lag)
            // Draw a radial gradient OVER the base colors to create the 3D look
            const gloss = ctx.createRadialGradient(
                x - radius * 0.35, y - radius * 0.35, radius * 0.1,
                x, y, radius
            );
            gloss.addColorStop(0, 'rgba(255, 255, 255, 0.8)'); // Shiny reflection point
            gloss.addColorStop(0.3, 'rgba(255, 255, 255, 0.1)'); // Soft center
            gloss.addColorStop(1, 'rgba(0, 0, 0, 0.4)'); // Deep spherical shadow
            
            ctx.fillStyle = gloss;
            ctx.beginPath(); ctx.arc(x, y, radius, 0, Math.PI * 2); ctx.fill();

            // 3. Crisp Separation Outline
    ...
    */
    ...
    // ⚪ PRICE LINE (Removed: Unnecessary, causes spiderweb clutter in Bookmap style)
    ...
    if (document.getElementById('showCandles')?.checked && klines.length > 0) {
        ...
        klines.forEach(k => {
            ...
            if (deltaVal !== 0 || isLive) {
                ...
                const badgeY = isGreen ? yl - gap - badgeH : yh + gap; 

                // Connecting Line
                ctx.strokeStyle = 'rgba(255,255,255,0.3)';
                ctx.lineWidth = 1;
                ctx.beginPath();
                ctx.moveTo(x, isGreen ? yl : yh);
                ...
                ctx.beginPath();
                // Compatible rounded rect (manual or simple rect for older mobile browsers)
                if (ctx.roundRect) {
                    ctx.roundRect(bx, by, badgeW, badgeH, 4);
                } else {
                    ctx.rect(bx, by, badgeW, badgeH); // Fallback to normal rect
                }
                ctx.fill();
                ctx.stroke();
                ctx.shadowBlur = 0;
    ...
    const cvdToggle = document.getElementById('showCVD');
    if (cvdToggle && cvdToggle.checked && cvdData.length > 2) {
        const cvdStartIdx = binarySearchIndex(cvdData, startTimeVisible - 5000);
        const visibleCVD = cvdData.slice(cvdStartIdx);
        
        if (visibleCVD.length > 2) {
            // 穩定化 CVD Scaling (Stable Scaling)
            // Use a slightly larger window than just visible to keep it stable
            const stableStartIdx = binarySearchIndex(cvdData, startTimeVisible - TIME_WINDOW_MS);
            const stableEndIdx = binarySearchIndex(cvdData, endTimeVisible + TIME_WINDOW_MS);
            const stableArea = cvdData.slice(stableStartIdx, stableEndIdx + 1);

            let minC = stableArea.length > 0 ? stableArea[0].value : visibleCVD[0].value;
            let maxC = stableArea.length > 0 ? stableArea[0].value : visibleCVD[0].value;
            
            stableArea.forEach(d => {
                if (d.value < minC) minC = d.value;
                if (d.value > maxC) maxC = d.value;
            });

            // Add padding for clean visualization
            const rangeC = Math.max(1, maxC - minC);
            minC -= rangeC * 0.1;
            maxC += rangeC * 0.1;
            
            // Update global scale for drawings
            currentCVDMin = minC;
            currentCVDMax = maxC;

            const cvdAreaH = Math.min(120, h * 0.2); // Limit height on small screens
            const cvdBTM = h - 30; // Position above time axis
            const cvdTop = cvdBTM - cvdAreaH;
            const getCVdy = (val) => cvdTop + (1 - (val - minC) / (maxC - minC)) * cvdAreaH;
            ...
            stableArea.forEach(d => {
                const x = getX(d.time);
                if (x < -50 || x > w + 50) return;
                const y = getCVdy(d.value);
                if (first) { ctx.moveTo(x, y); first = false; }
                else ctx.lineTo(x, y);
            });
            ctx.stroke();
            
            // Current CVD Label (Right Side)
            ctx.shadowBlur = 0;
            const lastCVD = visibleCVD[visibleCVD.length - 1];
            if (lastCVD) {
                const ly = getCVdy(lastCVD.value);
                ctx.fillStyle = '#f59e0b';
                ctx.font = 'bold 11px Inter';
                ctx.textAlign = 'right';
                ctx.fillText(`CVD: ${Math.round(lastCVD.value).toLocaleString()}`, w - sidebarW - 10, ly - 8);
                
                // Horizontal reference line to current value
                ctx.setLineDash([4, 4]);
                ctx.strokeStyle = 'rgba(245, 158, 11, 0.3)';
                ctx.beginPath();
                ctx.moveTo(0, ly);
                ctx.lineTo(w - sidebarW, ly);
                ctx.stroke();
            }
    ...
    renderDrawings(ctx, getX, getY, w, h);
}
